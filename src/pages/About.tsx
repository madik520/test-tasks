...
const About: FC<any> = (): ReactElement => {
  ...
  return (
    ...
    >
      { favorData.map((i) => {
        return <Card key={i.id}  sx={{ mt: 2, mr: { md: 3, sm: 2, xs: 0 }, maxWidth: 325 }}>
          <Link onClick={() => dispatch(selectCharacter({ ...i }))} style={{ color: 'black', textDecoration: 'none' }} to={`/character/${i.id}`}>
            <CardActionArea>
              <CardHeader
                avatar={
                  <Avatar aria-label="recipe" src={`${i.image}`} />
                }
                title={i.name}
          ...
          <Divider />
          <CardActions>
            <Typography color={"GrayText"} variant='body2' component={"span"}>Your favor character</Typography>
